反射的特征 : 动态性 ->在编译的时候无法确定要实例化哪一个类的实例,可以使用反射,在运行时创建对象
疑问:反射机制与面向对象中的封装特性是不是矛盾的?如何看待这两个技术?
    不矛盾: 反射机制解决的是能不能调用的问题,
            封装性更多的体现在建议,提示去调什么,怎么调用的作用上

反射 : 原来是从包->类->对象,现在是从对象可以获取到对象对应的类->类所在的包
反射的主要API都在Java.lang.reflect包中
在反射之前获取类实例-对象-->通过new关键字 -->在类的外部不可以通过实例对象调用其内部私有结构
反射可以调用类的私有化结构,有种方法/属性调用对象的感觉,实际是通过Mehtod对象和Field对象调用实例对象中的方法/属性
