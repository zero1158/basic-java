代理模式: 见springboot-Java项目

代理都分为三个部分 即 : 接口(客户类),被代理类(实现接口),代理类(实现接口)
    其中被代理类和代理类都必须实现同一个接口,这样才能用反射动态构建出需要的代理类对象
...
所谓代理:就是指a类需要调用c类的方法实现某个功能,
        ...
        a通过b类访问c类中的方法
这里我们将c类叫做目标类(被代理类),b类叫做代理类,a类叫做客户类
...
二 : 动态代理:
     见springboot-Java项目中有详细说明 --> Java基础中只是大概说了下,没有太多参考价值
